...
class ReferencePlugin(PluginBase):
    ...
    For example, we need to migrate directors and movies. We also need to keep a relationship between a movie and the
    director of that movie. Let's look on two CSV files:
    directors.csv:
        id, name
    ...
    Tivol keeps track of the ID from the source files, CSV, JSON or DB records, and know what is the ID of the record
    in the DB after the migration process completed.
    """
